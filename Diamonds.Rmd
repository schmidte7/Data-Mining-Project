...
diamonds$cut <- ordered(diamonds$cut, levels = c("Fair", "Good", "Very_Good", "Premium", "Ideal")) # Ordered from worst to best
...
```{r}
plot(diamonds$depth_ratio,diamonds$price)
```
...
```{r}
forecast::accuracy(predict(RegressTree, Train), Train$price)

forecast::accuracy(predict(RegressTree, Valid), Valid$price)
```
...
# Boosted tree
set.seed(111)
tree_boost10 <- gbm(price ~., data = Train, distribution = "gaussian", n.trees = 10)
tree_boost10
...
```{r}
ntree_opt_cv <- gbm.perf(tree_boost100, method = "cv")
...
```{r}
pred_boost <- predict.gbm(object = tree_boost100, newdata = Valid, n.trees = 100)

head(tree_boost100,14)
```

```{r}
forecast::accuracy(predict(tree_boost100, Valid), Valid$price)
```
...
```{r}
pred.imp <- varImp(tree_bagging)
pred.imp

#sort variable importance descending
VI_plot <- pred.imp[order(pred.imp$Overall, decreasing=TRUE),]

#visualize variable importance with horizontal bar plot
barplot(pred.imp$Overall,
        names.arg=rownames(pred.imp),
        horiz=TRUE,
        col='steelblue',
...
```{r}
forecast::accuracy(predict(tree_bagging, Valid), Valid$price)
```
...
forecast::accuracy(predict(rdf.model, Valid), Valid$price)
```
