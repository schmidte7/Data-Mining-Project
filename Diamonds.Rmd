...
```{r Error Details on Default Tree - Train}
# Predict errors using accuracy() 
tree_train_error <- forecast::accuracy(predict(RegressTree, Train), Train$price)
...
```{r Error Details on RegressTree2}
# Predict errors using accuracy() 
tree_train_error2 <- forecast::accuracy(predict(RegressTree2, Train), Train$price)
...
```{r Error Details on Prune_RegressTree2}
# Predict errors using accuracy() 
tree_train_error3 <- forecast::accuracy(predict(RegressTree_pruned, Train), Train$price)

kable(tree_train_error3) %>% kable_classic() 
```

```{r R-adjusted on Prune_RegressTree2}
# Calculate R^2 
p3 <- predict(RegressTree_pruned, Train)
R_sq3 <- (cor(Train$price,p3))^2
R_pct3 <- R_sq3*100
R_pct3
```

```{r Boosted Tree 10, fig.width = 5, fig.height = 5}
# Boosted tree
set.seed(111)
...
summary(tree_boost10, color = "#F9D53E")
```

```{r Error Details on tree_boost10}
# Predict errors using accuracy() 
tree_boost10_train <- forecast::accuracy(predict(tree_boost10, Train), Train$price)

kable(tree_boost10_train) %>% kable_classic() 
```

```{r Boosted Tree 30, fig.width = 5, fig.height = 5}
# Boosted tree
set.seed(111)
tree_boost30 <- gbm(price ~., data = Train, distribution = "gaussian", n.trees = 30)
tree_boost30

summary(tree_boost30)
```
...
```{r Boosted Tree 100, fig.width = 5, fig.height = 5}
# Boosted tree
set.seed(111)
...
summary(tree_boost100)
```

```{r Error Details on tree_boost100}
# Predict errors using accuracy() 
tree_boost100_train <- forecast::accuracy(predict(tree_boost100, Train), Train$price)
...
```{r Plot IV ,fig.width = 5, fig.height = 5}
pred_imp <- varImp(tree_bagging)

#sort variable importance descending
VI_plot <- pred_imp[order(pred_imp$Overall, decreasing=TRUE),]
...
```{r Errors Bagging}
tree_bagging_train <- forecast::accuracy(predict(tree_bagging, Train), Train$price)
...
```{r randomForest}
options(scipen = 9999)
rdf_model <- randomForest(price~ ., ntree= 100, data = Train)
plot(rdf_model)
```
...
```{r}
rdf_model_train <- forecast::accuracy(predict(rdf_model, Train), Train$price)
