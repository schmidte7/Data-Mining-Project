...
colnames(diamonds_dummies_cut)[10] = "cut_Very_Good"

dim(diamonds_dummies)
```

```{r Partitioning}
train_diamonds <- diamonds_dummies_cut[train.index,]
valid_diamonds <- diamonds_dummies_cut[valid.index,]
```
...
NNN <- cmpfun(train)
set.seed(111)
system.time(model <- keras_model_sequential() %>%
                layer_dense(units = 1, activation = "relu", input_shape = c(26)) %>%
                layer_dense(units = 1))
...
NeuralNetKeras <- model %>%
                  fit(x= train_diamonds_norm,
                      y =train_diamonds_norm$price,
                      epochs = 100,
                      batch_size = 32,
                      validation_split = 0.4)


# diaNeuralNet_1_1 <- tensorflow::train(price ~ .,
