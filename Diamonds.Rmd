...
```{r Plot KNN results, fig.height= 2.5, fig.width = 5}
plot3 <- data.frame("Real" = real_prices, 
                    "Predicted" = y_valid_pred, 
                    "Color" = diamonds[valid.index, 3],
                    "Cut" = diamonds[valid.index, 2],
                    "Clarity" = diamonds[valid.index, 4])
plot4 <- data.frame("Real" = real_prices, 
                    "Predicted" = y_valid_pred_2_26GLR, 
                    "Color" = diamonds[valid.index, 3],
                    "Cut" = diamonds[valid.index, 2],
                    "Clarity" = diamonds[valid.index, 4])


L1_N1_plot <- ggplot(data = plot3, aes(x= Real, y = Predicted, color = clarity))+
  geom_point(show.title = FALSE)+
  scale_color_brewer(type = 'div', guide = guide_legend(reverse = T, override.aes = list(alpha = 1, size = 0.1)))+
  labs(x= "NN (L1 N1)", y = "Predicted Values")+
  theme(legend.key.size = unit(0.5, 'cm'))
L1_N1_plot

L2_N26_plot <- ggplot(data = plot4, aes(x= Real, y = Predicted, color = clarity))+
  geom_point()+
  scale_color_brewer(type = 'div', guide = guide_legend(reverse = T, override.aes = list(alpha = 1, size = 0.1)))+
  labs(x= "NN (L2 N26", y = "")
L2_N26_plot

full_plot2 <- ggarrange(L1_N1_plot, L2_N26_plot, ncol=2, nrow=1, common.legend = TRUE, legend="right")
full_plot2
...
```{r Plot KNN results, fig.height= 4.5, fig.width = 3.5}
...
pred_plot_full <- ggarrange(pred_plot_lr, pred_plot_rg, pred_plot_knn, pred_plot_nn,
