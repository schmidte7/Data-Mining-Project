...
```{r threshold diff hist, include = TRUE, echo = FALSE}
...
```{r uncorrelated-ellipses, include=TRUE, echo=FALSE}
...
```{r hist-unskewed, include=TRUE, echo=FALSE}
...
```{r Best K Plot, include= TRUE, echo=FALSE}
#plot results
...
KNNRMSEplotdata <- t(knn_res$RMSE)
colnames(KNNRMSEplotdata) <- t(rownames(knn_res))

barplot(KNNRMSEplotdata, col = "#D8B365",border = "#D8B365" ,
...
```{r  RF Variable Importance, include = TRUE, echo = FALSE}
options(scipen = 9999)
...
```{r RegTree Summary, include = TRUE, echo = FALSE}
...
p<-ggplot(data=Tree_res, aes(x=RGRMSEplotdata, fill = "#D8B365" )) +
...
```{r NN Summary, include = TRUE, echo = FALSE}
...
```{r ensemble table, include = TRUE, echo = FALSE}
...
```{r Summ Color Plots, include = TRUE, echo = FALSE, dpi = 150}
...
```{r Summ Cut Plots, include = TRUE, echo = FALSE, dpi = 150}
