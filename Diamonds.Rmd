...
Train2 <- Train[, -c(6:8)]
LM2 = lm(price ~ ., data = Train2)
summary(LM2)
```


```{r}
# ProcStep(matx = Train[, -c(1)], y = Train$price, method = 'forward')
# ProcStep(matx = Train[, -c(1)], y = Train$price, method = 'backward')
GlobalCrit(LM)
GlobalCrit(LM2)
rm(Train2)
```
...
```{r}
LM3 = lm(price ~ . , data = Train[, c(1:6, 8, 9)])
summary(LM3)
LM4 = lm(price ~ . , data = Train[, c(1:5, 9)])
summary(LM4)
```
...
LM5 = lm(price ~. , data = TrainD)
summary(LM5)

TrainD2 <- TrainD[, -c(3:5)]
LM6 = lm(price ~ ., data = TrainD2)
summary(LM6)
```
