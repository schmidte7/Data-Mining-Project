...
```{r categorical bar plots, fig.height=3, fig.width=4, include=TRUE, echo=FALSE}
clarity <- ggplot(data = diamonds, aes(x = clarity)) +
    geom_bar(width = 0.5,
                 color = "#D8B365",
                 fill = "#D8B365") + ggtitle("Clarity") + # Title name
  ylab("Frequency")  + # Label names
  theme_classic() + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))

color <- ggplot(data = diamonds, aes(x = color)) +
    geom_bar(width = 0.5,
                 color = "#D8B365",
                 fill = "#D8B365") + ggtitle("Color") + # Title name
  ylab("") + # Label names
  theme_classic() + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))

cut <- ggplot(data = diamonds, aes(x = cut)) +
    geom_bar(width = 0.5,
                 color = "#D8B365",
                 fill = "#D8B365") + ggtitle("Cut") + # Title name
  ylab("") + # Label names
  theme_classic() + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))

# Arrange ggplots into one frame
ggarrange(clarity, color, cut, ncol = 3, nrow = 1, common.legend = TRUE, legend="right")
